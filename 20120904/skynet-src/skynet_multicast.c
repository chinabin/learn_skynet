...
skynet_multicast_copy(struct skynet_multicast_message *mc, int copy) {
	__sync_fetch_and_add(&mc->ref, copy);	// 将 mc->ref 加上 copy ，返回 mc->ref 之前的值
}
...
skynet_multicast_dispatch(struct skynet_multicast_message * msg, void * ud, skynet_multicast_func func) {
	...
	int ref = __sync_sub_and_fetch(&msg->ref, 1);	// 将 mc->ref 减 1 ，返回 mc->ref 修改之后的值
	if (ref == 0) {
		free((void *)msg->msg);
		free(msg);
	}
